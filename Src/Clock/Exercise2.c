...
#define RCC_CFGR2           *(volatile uint32_t*)(0x4002102CU) // Clock configuration register 2
...
void HCLK20M(void)
{
    ...
    RCC_CFGR |= (0 << 15); // PLL entry clock source ( HSI/2 used as PREDIV1 entry and PREDIV1 forced to div by 2. )
    RCC_CFGR |= (3 << 18); // PLL multiplication factor (PLL input clock x 5)
    ...
    RCC_CFGR |= (0 << 11);  // APB2 high-speed prescaler (HCLK not divided)
    RCC_CFGR |= (0 <<  8);  // APB1 high-speed prescaler (HCLK not divided)
    RCC_CFGR |= (0 <<  4);  // HLCK prescaler (SYSCLK not divided)
    ...
    RCC_CFGR |= (2 << 0);  // System clock switch (PLL selected as system clock)
...
void HCLK40M(void)
{
    /* step1: Enable required clock and wait until the clock is ready. 
        If application needs PLL, then configure the PLL and enable it */
    RCC_CFGR |= (0 << 15); // PLL entry clock source ( HSI/2 used as PREDIV1 entry and PREDIV1 forced to div by 2. )
    RCC_CFGR |= (8 << 18); // PLL multiplication factor (PLL input clock x 10)
    RCC_CR   |= (1 << 24); // PLL enable
    while(!((RCC_CR & 0x02000000) >> 25)); // 等待PLL開啟完成
    /* step2: Initialize CPU, AHB, APB busses clock prescalers to application requirements */
    RCC_CFGR |= (0 << 11);  // APB2 high-speed prescaler (HCLK not divided)
...
void HCLK60M(void)
{
    /* step1: Enable required clock and wait until the clock is ready. 
        If application needs PLL, then configure the PLL and enable it */
    RCC_CFGR |= (0 << 15); // PLL entry clock source ( HSI/2 used as PREDIV1 entry and PREDIV1 forced to div by 2. )
    RCC_CFGR |= (0xd << 18); // PLL multiplication factor (PLL input clock x 15)
    RCC_CR |= (1 << 24); // PLL enable
    while(!((RCC_CR & 0x02000000) >> 25)); // 等待PLL開啟完成
    /* step2: Initialize CPU, AHB, APB busses clock prescalers to application requirements */
    RCC_CFGR |= (0 << 11);  // APB2 high-speed prescaler (HCLK not divided)
    ...
    RCC_CFGR |= (0 <<  4);  // HLCK prescaler (SYSCLK not divided)
    /* step3: Configure the flash latency */
    FLASH_ACR |= (2 << 0);  // Flash Latency (Zero wait state, if 48 < HCLK ≤ 72 MHz)
    /* step4: Select newly enabled clock as SYSCLK */
    RCC_CFGR |= (2 << 0);  // System clock switch (PLL selected as system clock)
    while(((RCC_CFGR & 0x0c) >> 2) != 2); // 等待System clock switch完成
    MYUSART_Init();
